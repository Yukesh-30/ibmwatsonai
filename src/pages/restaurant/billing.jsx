...
const RestBilling = () => {
  ...
  const [products, setProducts] = useState([
    { id: "P001", name: "Rice (25kg)", barcode: "8901234567890", price: 1250.0, tax: 5, category: "Grains" },
    { id: "P002", name: "Wheat Flour (10kg)", barcode: "8901234567891", price: 450.0, tax: 5, category: "Grains" },
    { id: "P003", name: "Cooking Oil (5L)", barcode: "8901234567892", price: 650.0, tax: 18, category: "Oils" },
    { id: "P004", name: "Lentils Mix (5kg)", barcode: "8901234567893", price: 550.0, tax: 5, category: "Pulses" },
    { id: "P005", name: "Sugar (5kg)", barcode: "8901234567894", price: 250.0, tax: 5, category: "Essentials" },
    { id: "P006", name: "Salt (1kg)", barcode: "8901234567895", price: 20.0, tax: 5, category: "Essentials" },
    { id: "P007", name: "Tea Leaves (1kg)", barcode: "8901234567896", price: 300.0, tax: 18, category: "Beverages" },
    { id: "P008", name: "Milk Powder (1kg)", barcode: "8901234567897", price: 350.0, tax: 5, category: "Dairy" },
    { id: "P009", name: "Spice Mix (500g)", barcode: "8901234567898", price: 200.0, tax: 5, category: "Spices" },
    {
      id: "P010",
      name: "Vegetables Mix (5kg)",
  ...
  const processPayment = () => {
    if (paymentMethod === "cash" && Number.parseFloat(amountPaid) < calculateTotal()) {
  ...
  useEffect(() => {
    if (paymentMethod === "cash" && amountPaid) {
      setChange(Number.parseFloat(amountPaid) - calculateTotal())
    } else {
      setChange(0)
    }
  }, [amountPaid, paymentMethod])

  // Download Bill as PDF - FIXED AND ENHANCED
  const downloadBillPDF = async () => {
    if (cart.length === 0) {
      alert("No items in cart to generate bill!")
      return
    }
    ...
    try {
      ...
      const billData = {
        invoiceNumber,
        agencyName: agencyName || "Walk-in Customer",
        agencyContact,
        agencyEmail,
        cart,
      ...
      // Generate PDF
      const doc = pdfGenerator.generateBillPDF(billData)
  ...
  const sendBillEmail = async () => {
    if (!agencyEmail) {
      alert("Please enter agency email address")
      return
    }
    ...
    try {
      ...
      alert("📧 Email functionality would be implemented with your email service!")
    } catch (error) {
              ...
              <div className="lg:col-span-2">
                ...
                >
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <button onClick={() => setShowScanner(!showScanner)} className="btn btn-sm btn-outline">
                  ...
                  {showScanner && (
                      <div className="mb-4 p-4 border border-dashed border-gray-300 rounded-lg bg-gray-50">
                        <div className="flex flex-col items-center">
                          ...
                          {scannerResult && <p className="mt-2 text-sm text-gray-600">Last scanned: {scannerResult}</p>}
                        </div>
                      </div>
                  )}
                      ...
                      {filteredProducts.map((product) => (
                          <tr key={product.id} className="border-b hover:bg-gray-50">
                            ...
                            <td className="px-4 py-3">
                              <button onClick={() => addToCart(product)} className="btn btn-sm btn-outline btn-success">
                                <FiPlus size={14} />
              ...
              >
                ...
                <div className="mt-6 space-y-3">
                  ...
                  <button
                      ...
                      disabled={cart.length === 0 || isDownloading}
                      className="btn btn-outline btn-success w-full"
                  >
                    <FiDownload className="mr-2" />
                    {isDownloading ? "Generating PDF..." : "Download Bill PDF"}
                  </button>

                  <button onClick={resetBilling} className="btn btn-outline btn-error w-full">
                    ...
                    {paymentComplete ? (
                        ...
                        <>
                            ...
                            {paymentMethod === "cash" && (
                                <div className="mb-4">
                                  ...
                                  <input
                                      ...
                                      step="0.01"
                                  />
                                  {Number.parseFloat(amountPaid) >= calculateTotal() && (
                                      <p className="text-sm text-green-600 mt-1">
                                        Change: {formatCurrency(Number.parseFloat(amountPaid) - calculateTotal())}
                                      </p>
                                  )}
                                </div>
                            )}
                          ...
                          <div className="flex space-x-3">
                            <button onClick={() => setShowPaymentModal(false)} className="btn btn-outline flex-1">
                              Cancel
                            </button>
                            <button
                  ...
                  >
                    <div className="flex justify-between items-center mb-6">
                      ...
                      <div className="flex space-x-2">
                        <button onClick={printBill} className="btn btn-sm btn-outline">
                          <FiPrinter className="mr-1" size={14} />
                          Print
                        </button>
                        <button onClick={downloadBillPDF} disabled={isDownloading} className="btn btn-sm btn-primary">
                          <FiDownload className="mr-1" size={14} />
                          {isDownloading ? "Downloading..." : "Download PDF"}
                        </button>
                        <button
                            onClick={sendBillEmail}
                            disabled={isSendingEmail || !agencyEmail}
                    ...
                    <div className="border rounded-lg p-6">
                      ...
                      <div className="mb-8">
                        ...
                        {agencyEmail && <p className="text-gray-600">{agencyEmail}</p>}
                      </div>
                    ...
                    <div className="mt-6 flex justify-end">
                      <button onClick={resetBilling} className="btn btn-primary">
                        New Bill
                      </button>
                    </div>
                  </motion.div>
